...
apply_terminal_theme() {
    ...
    # Parse the palette from GNOME Terminal format ['rgb(r,g,b)',...] to hex array
    local PALETTE_CLEAN=$(echo "$PALETTE" | tr -d "[]' ")
    local -a KITTY_COLORS=()
    local i=0
    IFS=',' read -ra RGB_PARTS <<< "$PALETTE_CLEAN"
    for part in "${RGB_PARTS[@]}"; do
        if [[ "$part" =~ rgb\(([0-9]+),([0-9]+),([0-9]+)\) ]]; then
